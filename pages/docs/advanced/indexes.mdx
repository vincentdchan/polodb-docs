...
Indexes support the efficient execution of queries in PoloDB.
...
use polodb_core::{
    bson::{doc, Document},
    CollectionT, Database, IndexModel, Result,
};

fn create_age_index(db: &Database) -> Result<()> {
    let teachers = db.collection::<Document>("teacher");
...
use polodb_core::{
    bson::{doc, Document},
    CollectionT, Database, IndexModel, IndexOptions, Result,
};

fn create_unique_name_index(db: &Database) -> Result<()> {
    let teachers = db.collection::<Document>("teacher");
...
```rust
use polodb_core::{bson::Document, CollectionT, Database, Result};

fn drop_age_index(db: &Database) -> Result<()> {
    let teachers = db.collection::<Document>("teacher");
